from RNNbased.data import load, prepare_train, prepare_pred, one_hot
from RNNbased.cleansing import get_stopwords, get_wordlists
from RNNbased.tokenizing import set_tokenizer, tokenize, check_freq
from RNNbased.padding import plot_sentences, check_len, pad_sentences
from RNNbased.modeling import create_LSTM, create_GRU
from RNNbased.utils import train_model, plot_history, best_model, save_info
from RNNbased.predicting import pred_data, get_model
import os
import numpy as np
import pandas as pd
...
# load data for train
(X_train_raw, y_train_raw), (X_test_raw, y_test_raw) = prepare_train(TRAIN_PATH)
...
# save_info(f"{output_path}/NoUsemodels", )
...
X_pred_raw = prepare_pred(PRED_PATH, filter=True, verbose=1)
print(X_pred_raw[:10])
